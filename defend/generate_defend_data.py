...
class Defender:
    def __init__(self, modify_source_operation: str = "remove", save_data_dir : str = "", word2vec_model_path: str = "",
        ...
        if self.operation == "replace_wordnet_synonym":
        ...
        for idx_data, (s_data, t_data) in enumerate(zip(source_data, target_data)):
            ...
            if self.operation == "remove":
                for idx_token in range(len(s_data_tokens)):
                    variant_s_tokens = s_data_tokens[: idx_token] + s_data_tokens[idx_token+1:]
                    f_source.write(f"{' '.join(variant_s_tokens)}\n")
                    f_target.write(f"{t_data}\n")
                    f_idx.write(f"{str(idx_data)}\n")
            elif self.operation == "replace_wordnet_synonym":
                for idx_token in range(len(s_data_tokens)):
                    token = s_data_tokens[idx_token]
                    if token in self.word2synonym_dict.keys():
                        variant_s_tokens = s_data_tokens[: idx_token] + random.sample(self.word2synonym_dict[token.lower()],1) + s_data_tokens[idx_token+1:]
                    else:
                        variant_s_tokens = s_data_tokens[: idx_token] + ["<unk>"] + s_data_tokens[idx_token+1:]
                    f_source.write(f"{' '.join(variant_s_tokens)}\n")
                    f_target.write(f"{t_data}\n")
                    f_idx.write(f"{str(idx_data)}\n")
