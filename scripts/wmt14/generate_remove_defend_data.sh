...
# file: generate_remove_defend_data.sh
...
--gen-subset "valid1" \
--path ${MODEL_PATH} \
--batch-size ${BATCH} --beam ${BEAM} --lenpen ${LENPEN} --remove-bpe --skip-invalid-size-inputs-valid-test > ${SAVE_PRED}/target_valid_attacked.log
...
CUDA_VISIBLE_DEVICES=${GPUID} fairseq-preprocess --source-lang en --target-lang de \
      --srcdict ${CORPUS_DICT}  \
